...
while (number <= 1000) {
  ...
  console.log('Generating invite....')
   ...
   var inv = await chan.createInvite({maxAge:0,unique:true},'Invite Generator')
      if (inv.toString().indexOf('dot') > -1 || inv.toString().indexOf('now') > -1 || inv.toString().indexOf('apple') > -1  || inv.toString().indexOf('tech') > -1  || inv.toString().indexOf('dis') > -1  || inv.toString().indexOf('gg') > -1) {
